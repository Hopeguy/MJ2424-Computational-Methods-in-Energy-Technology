%% 3 STEADY STATE TEMPERATURE USING 3 Central Diff
...
prompt =  {'Input values for step size dx:', 'Input values for step size dy'};
dlgtitle = 'Input';
definput = {'0.0125', '0.0125'};
dims = [1 50];
%Step size
Input_matrix = inputdlg(prompt, dlgtitle, dims, definput);
dx = (str2double(Input_matrix(1,1)));
dy = str2double(Input_matrix(2,1));
...
for i = 1:1:length(Vect_1_b)
    ...
    if i == 1 %Left top corner
        ...
        Matrix_1_A(i,i) = -4;
        Matrix_1_A(i,i+1) = 1;
        Matrix_1_A(i, i+(N-2)) = 1;
        
        Vect_1_b(i) = Matrix_1(1,2) + Matrix_1(2,1);
    end
    ...
    if i == (N - 2)    %right top corner
        ...
        Vect_1_b(i) = Matrix_1(1,N -1) + Matrix_1(2,N);
    end
    ...
    if i == ((N-2)*(M-3)) + 1    %left down corner
        ...
        Vect_1_b(i) = Matrix_1(M -1 ,1) + Matrix_1(M,2);
    end
    ...
    if i == (N-2)*(M-2)      %Right down corner
        ...
        Vect_1_b(i) = Matrix_1(M,N -1) + Matrix_1(M - 1 ,N);
    end
...
X1 = ['At first point ', num2str(Matrix_1(y,x1)), ' Celsius'];
X2 = ['At second point ', num2str(Matrix_1(y,x2)), ' Celsius'];
X3 = ['At third point ', num2str(Matrix_1(y,x3)), ' Celsius'];
...
%% Question 3: 5 Point Central differention Temp Steady state
...
%Starts a program that ask for the value of dx and dy
prompt =  {'Input values for step size dx:', 'Input values for step size dy'};
dlgtitle = 'Input';
definput = {'0.0125', '0.0125'};
dims = [1 50];
...
for f = 0:1:(M - 7)
    ...
    Matrix_1_A(i,i+1) = 16;
    Matrix_1_A(i,i-1) = 16;
    Matrix_1_A(i,i+(N-2)) = 16;
    Matrix_1_A(i,i-(N-2)) = 16;
    %Second out points
    Matrix_1_A(i,i+2) = -1;
    Matrix_1_A(i,i-2) = -1;
    Matrix_1_A(i,i-(2*(N-2))) = -1;
    Matrix_1_A(i,i+(2*(N-2))) = -1;
...
for i = 1:1:length(Vect_1_b)
    ...
    if i == N %Left top corner
        ...
        %Outer points
        Matrix_1_A(i,i+2) = -1;
        Matrix_1_A(i,i+((N-2)*2)) = -1;
     
        
        Vect_1_b(i) = -(Matrix_1(1,3) - Matrix_1(3,1));
    ...
    if i == N+(N-5)  %right top corner
        ...
        Vect_1_b(i) = -Matrix_1(1,N -2) - Matrix_1(3,N);
    end
    ...
    if i == ((N-2)*(M-4)) + 2   %left down corner
        ...
        Vect_1_b(i) = -Matrix_1(M-2,3) - Matrix_1(M,3);
    end
    ...
    if i == (N-2)*(M-3) -1      %Right down corner
        ...
        Vect_1_b(i) = -Matrix_1(M-2,N) - Matrix_1(M,N-2);
    end
